    ...
    <div className={`${styles.hiLabsContainer} pt-16 md:pt-24 pb-20`}>
        ...
        {Object.keys(REGION_INFO).map((key) => {
          ...
          return (
            <div
              ...
              style={{borderColor: active ? '#2DD4BF' : undefined}}
            >
      ...
      <div style={{maxWidth:'640px',margin:'0 auto 2rem',display:'flex',gap:'0.5rem',alignItems:'center',paddingLeft:'1.5rem',paddingRight:'1.5rem'}}>
        <div style={{position:'relative',flex:1}}>
          ...
          <Search size={18} style={{position:'absolute',left:10,top:'50%',transform:'translateY(-50%)',color:'#2DD4BF'}} aria-hidden />
          <input
            id="labSearch"
            value={searchQuery}
            onChange={e=>setSearchQuery(e.target.value)}
            placeholder="Search labs by name or description"
            style={{width:'100%',padding:'0.75rem 0.75rem 0.75rem 2.2rem',borderRadius:8,border:'1px solid rgba(45, 212, 191,0.3)',background:'rgba(45, 212, 191,0.05)',color:'#fff',outline:'none'}}
          />
        </div>
        {searchQuery && (
          <button onClick={()=>setSearchQuery('')} style={{padding:'0.65rem 0.9rem',borderRadius:8,border:'1px solid rgba(45, 212, 191,0.3)',background:'rgba(45, 212, 191,0.1)',cursor:'pointer',color:'#2DD4BF'}} aria-label="Clear search">Clear</button>
        )}
      </div>
      ...
      <div className={`${styles.dashboardPromo} px-6`}>
        <div className={`${styles.promoContent} max-w-5xl mx-auto`}>
          ...
          <a href={asset("/Lab-documents/demo.pdf")} download className={styles.downloadButton} aria-label="Get sample report">Get Sample Report ⬇️</a>
        </div>
      </div>
    </div>
  );
