...
export default function HIWorkshopsPage() {
  const [activePathway, setActivePathway] =
    useState<(typeof PATHWAYS)[number]>('All');
  ...
  return (
    <div className="min-h-screen bg-[#0B1630] pt-16 md:pt-24 pb-20">
      
      {/* ===== SEO (NON-VISUAL) ===== */}
      <h1 className="sr-only">
        Human Intelligence Workshops for Cognitive Training, Psychology and Innovation
      </h1>

      <section className="sr-only">
        <h2>Human Intelligence Training Workshops</h2>
        <p>
          Aaruchudar Human Intelligence Workshops focus on cognitive development,
          psychology-based thinking, neuroscience principles, and innovation skills.
          These workshops are designed to improve clarity, decision making,
          emotional intelligence, and advanced thinking abilities.
        </p>
      </section>
      {/* ===== END SEO ===== */}
      ...
      <div className="max-w-7xl mx-auto px-6">
        <Link
          href="/"
        className="absolute top-6 left-6 z-20 border border-brand/40 text-brand-light flex items-center gap-2 px-4 py-2 rounded-md bg-transparent hover:bg-brand hover:text-white transition"
      ...
      <header className="relative flex items-center justify-center overflow-hidden py-24 md:py-28 px-6 min-h-[54vh]">
        ...
        <div className="relative z-10 max-w-5xl mx-auto text-center">
          ...
          >
            HI{' '}
            <span className="bg-gradient-to-r from-brand-light via-neural-cyan to-growth-mint bg-clip-text text-transparent">
              Workshops
            </span>
          </motion.h1>
          ...
          <motion.div className="mt-8 flex flex-wrap justify-center gap-3">
            <span className="px-3 py-1 rounded-full text-xs md:text-sm border border-white/10 bg-white/5 text-gray-200">
              ...
              <button
                ...
                className={`px-3 py-2 rounded-md text-sm border transition ${
                  activePathway === p
                    ...
                    : 'border-white/10 text-gray-300 bg-white/5 hover:border-neural-cyan/40 hover:text-white'
                }`}
              >
                {p}
              </button>
      ...
      <main className="py-12 md:py-14 px-6">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 max-w-7xl mx-auto">
          {filteredWorkshops.map((w, index) => (
            <motion.article
              key={w.title}
              className="rounded-2xl p-7 flex flex-col backdrop-blur-md shadow-xl bg-white/5 border border-white/10 hover:border-neural-cyan/40 transition hover:-translate-y-2"
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {w.duration && (
                  <span className="px-3 py-1 text-xs rounded-full border border-white/10 bg-white/5 text-gray-300 flex items-center gap-2">
              ...
              <ul className="space-y-3 mb-8">
                {w.items.map((item) => (
                  <li key={item} className="flex gap-3 text-sm text-gray-300">
                    <span className="w-2 h-2 mt-2 rounded-full bg-neural-cyan" />
                    {item}
                  </li>
                ))}
              </ul>
