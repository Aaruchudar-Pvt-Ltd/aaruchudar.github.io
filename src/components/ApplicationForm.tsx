...
export default function ApplicationForm({ roles, locations }: ApplicationFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const body = [
      ...
      `Resume: ${form.resume ? form.resume.name + " (please attach this PDF to the email)" : "Not attached"}`,
      "",
      "Why should we hire you?",
      form.reason,
    ].join("\n");

    openMailto(`Job application: ${form.role} — ${form.name}`, body);
    setSuccess("Opening your email app. Please attach your resume PDF before sending.");
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl border border-slate-200 bg-white p-5 md:p-6 shadow-sm">
      ...
      <div className="mt-6 flex items-center gap-3">
        <button
          type="submit"
          className="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-5 py-2.5 text-white font-medium shadow-sm hover:bg-indigo-700"
        >
          Submit Application
        </button>
