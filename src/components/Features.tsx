...
const ProgramCard = ({ data }: { data: any }) => (
  <div className="group flex flex-col h-full rounded-xl overflow-hidden border border-brand/15 backdrop-blur-xl bg-white shadow-sm transition-all duration-300 hover:shadow-md hover:-translate-y-[2px] hover:border-brand/50">
    {/* Image: reduce height */}
    <div className="w-full h-24 overflow-hidden">
      <img src={asset(data.image)} alt={data.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
    </div>
    {/* Content: tighter paddings & smaller typography */}
    <div className="p-3 flex flex-col flex-1">
      <div className="flex items-center gap-2 mb-2">
        <img src={asset(data.icon)} alt={data.title} className="w-6 h-6 opacity-90" />
      ...
      <div className="mt-auto flex gap-2 pt-2 border-t border-white/50">
        <Link
          href={data.href}
          className="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-full bg-brand text-white text-[11px] font-semibold shadow-sm hover:bg-brand-light"
        >
          Explore
          <svg
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex sm:hidden justify-center gap-4 mt-2">
          <button
            aria-label="Previous"
            onClick={() => scrollByViewport(-1)}
            className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full border border-brand/30 bg-white text-brand shadow hover:bg-blue-surface text-sm"
          >
        ...
        <div className="mt-16">
          {/* IQ Banner */}
          <div className="relative overflow-hidden rounded-2xl border border-brand/20 bg-gradient-to-r from-white via-blue-surface to-mint-surface shadow-lg">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-0 items-center">
              ...
              <div className="p-6 sm:p-10">
                <h3 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900">
                  ...
                  <span className="block text-brand">Explore Neuro Tech</span>
                </h3>
                <p className="mt-3 text-slate-600 max-w-md">A single system that measures, trains, and improves your mind every day.</p>
                <div className="mt-6">
                  <Link href="/neuro" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-brand text-white text-sm font-semibold shadow hover:bg-brand-light transition-colors">
                    Explore Neuro Tech
              ...
              <div className="relative p-6 sm:p-10 flex items-center justify-center">
                <img
                  src={asset("/images/neuro_tech_suite.jpg")}
                  alt="Neuro Tech Suite"
                  className="max-h-40 sm:max-h-56 md:max-h-64 w-auto rounded-xl border border-brand/20 shadow-md"
                />
              </div>
            </div>
          </div>
